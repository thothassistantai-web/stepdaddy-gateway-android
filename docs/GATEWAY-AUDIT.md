...
Fresh install / clear-data loads **full DaddyLive catalog** plus **all supplements** (Special Events, iptv-org, Free-TV, Dulo Live, ntv.cx, Adult Swim) with `FULL_CATALOG` import modes (every provider row as its own playlist entry). MoveOnJoy / TheTvApp / xyzstreams / TVPass were removed.
